...
phase: 0
always_active: true
absorbs: SECURITY/secrets-management
description: "Secret discipline — .env, vault, rotation, leak detection. Never commit credentials. Secrets never in state files."
keywords: [segredo, secret, API-key, token, .env, credencial, password, vault, leak, state file, .sota]
---

# Secrets Guard

> Phase 0 — ALWAYS ACTIVE. The only security skill that runs on every interaction.

This skill enforces absolute discipline around secret handling: environment variables, API keys, tokens, passwords, and credentials. Secrets must never appear in code, version control, state files, logs, or any observable system state.
...
### State Files Must Never Contain Secrets

Secrets must NEVER appear in `.sota/` directory or any other state files (`.state`, `.tfstate`, `.json` config snapshots, etc.):

```bash
# BAD - do not commit this pattern
# .sota/state.json contains: { "api_key": "sk_live_xxx" }

# GOOD - externalize all secrets
# .sota/state.json references only: { "api_key_ref": "vault://prod/api_keys" }
```

Use secret managers for state persistence. Never snapshot credentials in any file system location.
